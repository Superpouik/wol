class ComfyUIInterface {
    ...
    switchTab(tabName) {
        ...
        // Charger les modèles si on accède à Quick Gen et qu'ils ne sont pas encore chargés
        if (tabName === 'quickgen') {
            console.log('🎯 Accès à Quick Gen - vérification des modèles...');
            const hasModels = this.availableModels.controlnets.length > 0 || 
                            this.availableModels.checkpoints.length > 0;
            
            if (!hasModels) {
                console.log('📦 Aucun modèle chargé, déclenchement de loadModels()...');
                this.loadModels();
            } else {
    ...
    displayResult(imageInfo) {
        ...
        if (imageUrl) {
            resultContainer.innerHTML = `
                <img src="${imageUrl}" alt="Image générée" class="result-image" onclick="this.requestFullscreen()">
            `;
        } else {
            resultContainer.innerHTML = `
                <div class="error-message">
